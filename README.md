...
This is a proof of concept project to showcase that you can run various applications or plugins on the server side without requiring that the admin installs each application on the server.
...
In the spirit of the free (libre) software movement, a version could be deployed where server side code is minimized as much as possible, and plugin code would get passed in signed, but unencrypted.

As a more extreme example, only a deep link to a signed git commit in a public source code repository would be passed in with the request. The server could build and cache the source code itself as well as the visitors of the website. The visitor could choose to invoke the version of the code they wish, as long as it is not on the blocklist. The visitor could also decide to fork the code to host an improved version themselves. Extra care must be exercised to ensure database schema compatibility.

* https://www.gnu.org/philosophy/who-does-that-server-really-serve.html

### Transparency

In general, I like to know what happens when I click a link before I open it. In case of dynamic web apps, the highest form of transparency is if you include the server side source code of the app itself in the URL.

* https://en.wikipedia.org/wiki/Nothing-up-my-sleeve_number

### Link rot: Finding alternatives

Links can and should be transparent about what content they refer to by including the title and as many relevant keywords as feasible in the URI formatted for easy tokenization. E.g.,

* http://example.com/execute-signed-backend-code-from-URI#technical-background

This can help viewers find mirrors or reposts of the referenced content elsewhere using a web search engine in case the originally referenced instance went offline.

### Link rot: Archiving

Our links can cease to work one day to another if maintaining to run the complicated backend serving the page proves to be too expensive or the machine fails. Web archiving crawlers can also have a hard time saving the content of portals generated by complicated logic.

In contrast, if even a single link remains that points towards the service (the dead link itself), anyone can resurrect the service from that URI. The few lines of code of the handler that executes the URI on the server also prints itself out if it is invoked without an URI to execute. However, plugins are usually implemented in a way that they could also run on a bare server just by copy & pasting the unescaped code into a file.

### Feature creep

Imposing limitations towards the implementation, similarly to the rules of the demoscene, results in ingenuity. It helps define and keep a narrow product focus while avoiding feature creep.

8kB can be assumed as an absolute maximum URI length, while 2-4kB can be considered a safer upper bound - see below for details on compatibility.

After practising such extreme size coding (golfing), you will question the sole existence of most code lines in a typical application backend. And the less lines of code, the less potential bugs and the lower the maintenance costs.
...
To avoid opportunistic snooping and automated scanning by your shared host.
...
* Usually not high for short code snippets in scope of this project. Typical TLS/HTTP request overheads are in the same ball park.
...
* Enter either the folder `rsa` or `rsa-aes` depending on whether you want to encrypt the plugin in addition to signing it
* Optionally add the URL of your server to `var/hostname.txt`
* `php linkify.php`
...
If you use the resulting parameter in an HTTP GET for testing (like in an `<img src=...?p=...>`), it will reveal your key in the access logs of the server! So, you should do an HTTP POST with the encrypted code instead using one of the following:
...
You should host all sources containing this code on a separate, possibly static web server. It does not make sense to store the encryption key next to the encrypted data itself, revealing both at the same time in case of a compromise.
...
* brotli compresses much better than gzip even for such tiny input
...
* Chromium and Firefox
  ...
  * Can confuse some Markdown parsers such as Matrix Element: `[][*_]`
* References
...
* When using AJAX, binary could be sent directly.
...
* TODO: can some kind of scheme for privacy or integrity be devised using `.htaccess` and SSI for static web servers?
